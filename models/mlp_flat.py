...
class MLPClassifier(pl.LightningModule):
    def __init__(self,
        ...
        super().__init__()
        self.hidden_layers = [
            nn.Linear(vector_size * 50, hidden_layers[0])
        ]
...
def train():
    ...
    trainer = pl.Trainer(max_epochs=1, progress_bar_refresh_rate=20, gpus=1, logger=mlf_logger, callbacks=[early_stopping], default_root_dir=BASE_PATH + "mlp_flat/")
    trainer.fit(model)
    trainer.test()
    return model
